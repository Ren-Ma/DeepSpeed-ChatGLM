...
class gpt4llm(PromptRawDataset):

    def __init__(self, output_path, seed, local_rank, dataset_name):
        super().__init__(output_path, seed, local_rank, dataset_name)
        self.dataset_name = "unidt_FAQ"
        self.dataset_name_clean = "unidt_FAQ"
        if "xlsx" in dataset_name:
            df = pd.read_excel(dataset_name)
        elif "csv" in dataset_name:            
            df = pd.read_csv(dataset_name)
        else:
            raise FileNotFoundError("dataset_name is not csv nor excel file.")
        df.columns = ["prompt", "chosen", "rejected"]
        train, test = train_test_split(df, test_size=200, random_state=seed)
        train_dataset = Dataset.from_dict(train)
        test_dataset = Dataset.from_dict(test)
        self.raw_datasets = DatasetDict({"train":train_dataset,"test":test_dataset})
