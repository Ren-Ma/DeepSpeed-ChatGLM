...
# wandb.init(project = "deepspeed-chat-3", name = "unidt_FAQ_0525")
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    prompt_train_dataset, _ = create_prompt_dataset(
        ...
        args.max_prompt_seq_len, end_of_conversation_token="")
...
def main():
    ...
    if args.model_class == 'chatglm':
        tokenizer = AutoTokenizer.from_pretrained(args.actor_model_name_or_path, trust_remote_code=True)
    else:
    ...
    for epoch in range(args.num_train_epochs):
        print_rank_0(
            ...
            args.global_rank)
        print_rank_0(
            f"Number of steps {len(prompt_train_dataloader)}, {len(unsupervised_train_dataloader)}",
            args.global_rank) 
        # 经过dataloader generate后，DataCollatorRLHF.__call__函数会把batch中的input_ids和attention_mask再次反转，所以最后得到的跟未反转前一致
        for step, (batch_prompt, batch_unsupervised) in enumerate(
                tqdm(zip(prompt_train_dataloader, unsupervised_train_dataloader))): 
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        #         'Train/actor_loss': actor_loss_sum / inner_iter,
                        #         'Train/critic_loss': critic_loss_sum / inner_iter,
                        #         'Train/average_reward': average_reward / inner_iter
                        #     })

                        #     if step % 100 == 0:
                        #         prompts = out["prompts"]
                        #         outputs = out["input_ids"]

                        #         decoded_prompts = [tokenizer.decode(tokens, skip_special_tokens=True) for tokens in prompts]
                        #         decoded_generated = [tokenizer.decode(tokens, skip_special_tokens=True) for tokens in outputs]

                        #         # Create a wandb.Table with column names
                        #         example_table = wandb.Table(columns=["Prompt", "Generated Response"])

                        #         # Add rows to the table
                        #         for prompt, gen in zip(decoded_prompts, decoded_generated):
                        #             example_table.add_data(prompt, gen)
                        #         # actor_lr, critic_lr = trainer.get_learning_rate()

                        #         # Log the table to wandb
                        #         wandb.log({"Eval/examples": example_table, "Eval/step": step})


                    random.shuffle(exp_dataset)
                    random.shuffle(unsup_dataset)

                # print_rank_0(f"""epoch: {epoch}|step: {step}|ppo_ep: {ppo_ep+1}|
                #              act_loss: {actor_loss_sum/inner_iter}|
                #              cri_loss: {critic_loss_sum/inner_iter}|
                #              unsuper_loss: {unsup_loss_sum/inner_iter}""", args.global_rank)
                # average_reward = get_all_reduce_mean(average_reward).item()
                # print_rank_0(f"average reward score: {average_reward/inner_iter:.2f}", args.global_rank)
    ...
    if args.output_dir is not None:
        ...
        # save_zero_three_model() 主要覆盖了save_hf_format()时保存的actor和critic的pytorch_model.bin文件
