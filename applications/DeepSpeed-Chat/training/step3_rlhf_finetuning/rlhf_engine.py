...
from transformers import AutoModelForCausalLM, get_scheduler, AutoModel, AutoConfig
...
from utils.model.model_utils import create_hf_model, create_critic_model, create_chatglm_model, create_chatglm_critic_model
...
class DeepSpeedRLHFEngine():
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        # Model
        if self.args.model_class == 'chatglm':
            actor_model = create_chatglm_model(
            ...
            # actor_model = AutoModel.from_pretrained(
            #     actor_model_name_or_path,
            #     trust_remote_code=True,
            #     config=model_config)
        else:
    ...
    def _init_ref(self, actor_model_name_or_path):
        ...
        if self.args.model_class == 'chatglm':
            ref_model = create_chatglm_model(
                model_class=AutoModel,
                model_name_or_path=actor_model_name_or_path,
                tokenizer=self.tokenizer,
                ds_config=ds_config, 
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        # Model
        if self.args.model_class == 'chatglm':
            critic_model = create_chatglm_critic_model(
    ...
    def _init_reward(self, critic_model_name_or_path):
        ...
        # Model
        if self.args.model_class == 'chatglm':
            reward_model = create_chatglm_critic_model(
                model_name_or_path=critic_model_name_or_path,
                tokenizer=self.tokenizer,
                ds_config=ds_eval_config,
                num_padding_at_beginning=self.args.num_padding_at_beginning,
